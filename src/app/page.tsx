...
export default function Home() {
  ...
  const fetchTodayGrid = useCallback(async () => {
    try {
      const response = await api.get<PuzzleGrid>('api/v1/grids/today');

      setTodayGrid(response.data);
    } catch (error) {
        ...
        <div className="flex flex-col md:mr-32 place-items-center mt-16">
          ...
          <div className="flex flex-row">
            ...
            {todayGrid?.grid_tips?.map((tip) => {
              if (tip.i !== null) {
                return (
                  <div
                    className="w-24 md:w-40 h-20 text-center content-end pb-4"
                    key={tip.id}
                  >
                    {tip.description}
                  </div>
          ...
          <div className="flex flex-row">
            <div className="flex flex-col mr-3">
              {todayGrid?.grid_tips?.map((tip) => {
                if (tip.j !== null) {
                  return (
                    <div
                      className="size-24 md:size-40 text-center content-center"
                      key={tip.id}
                    >
                      {tip.description}
                    </div>
            ...
            <div className="border border-slate-200">
              {todayGrid &&
                Array.from({ length: todayGrid.grid_rows }).map(
                  (_item, index, { length }) => (
                    <div
                      className={`flex flex-row ${length - 1 !== index ? 'border-b border-slate-200' : ''}`}
                      key={index}
                    >
                      {Array.from({ length: todayGrid.grid_columns }).map(
                        (_item, index) => (
